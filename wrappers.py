...
class GPhoto(Wrapper):
    ...
    def capture_image_and_download(self, shot):
        print str("Shot nr:")        
        print str(shot)
        code, out, err = self.call(self._CMD + " --capture-image-and-download --filename="+ str(shot))
        print str("Test")
        if code != 0:
            raise Exception(err)
    ...
    def set_shutter_speed(self, secs=None, index=None):
        ...
        if secs:
            ...
            print "done"
        if index:
            print "Index it is and a speed of!"
            print str(index)
            code, out, err = self.call([self._CMD + " --set-config /main/capturesettings/shutterspeed=" + str(index)])



    def set_bracketing(self):
        code, out, err = None, None, None
        code, out, err = self.call([self._CMD + " gphoto2 --set-config /main/capturesettings/bracketing=0"])
##        code, out, err = self.call([self._CMD + " gphoto2 --set-config /main/other/d0c3=8"])
        print "Bracketing Set"
